...
string analyse_file_req(char* buf,int* NIDS,int* NPORTS,string* SFILES,int NUM_FILES,int NUM_NEIGHBOURS,int exp_seq){
...
int main(int argc, char* argv[] ){
	...
	// ----------------------------------------------- part5 : ANALYSING ALL THE FILES INFO AND GENERATING REPLIES FOR NEXT CONNECTION.
		int DEPTHS[NUM_FILES];
		int UIDS[NUM_FILES];
		int OINDEXS[NUM_FILES];

		string file_replies[NUM_NEIGHBOURS];	//--which files we need from a given neighbour... given as bit string 0101001
		for (int i = 0; i < NUM_NEIGHBOURS; ++i)
		{
				file_replies[i] = "";
		}
		...
		for (int n = 0; n < NUM_FILES; ++n)
		{
			...
			int depth = 0;
			int index = 0;	//*--index of neighbour.(i)
			int uid = 0;
			...
			for (int i = 0; i < NUM_NEIGHBOURS; ++i)
			{
				file_replies[i] += to_string(depth*(index == i));
			}
		}


	// ----------------------------------------------- part6 : creating Downloaded Directory.
		string path = string(argv[2]) + "/Downloaded";
		mkdir(&path[0],0777);

	// ------------------------------------------------------------------------PART7 :  FILE TRANSFERRRRRRRR.
		...
		for( int i = 0 ; i < NUM_NEIGHBOURS; i++){
			...
			if(NIDS[i] > MY_ID){
				...
				// --- RECEIVING HIS REPLY.
					...
					req_str = analyse_file_req(buf,NIDS,NPORTS,SFILES,NUM_FILES,NUM_NEIGHBOURS,3);
			...
			//else You send the requrst to your neighbour for conection,
					...
					// --receiving his ID + FILE REQUEST.
						...
						int len_buf = 200;
						int bytes_rcved = recv(sockfd_send[i], buf, len_buf , 0);
						req_str = analyse_file_req(buf,NIDS,NPORTS,SFILES,NUM_FILES,NUM_NEIGHBOURS,4);
	...
	// -------------------------------------------------------------------------- PRINTING MD5 HASHES OF DOWNLOADED FILES.
	int file_descript;
	...
	for (int i = 0; i < NUM_FILES; ++i)
	{
		if( DEPTHS[i] != 0 ){
		    ...
		    munmap(file_buffer, file_size); 
		  	...
		  	cout << "Found " << SFILES[i] <<  " at " << UIDS[i] << " with MD5 "; 
